...
//            Log.d("TelemetryPlayerActivity",contentTelemetry.toString());
            ...
            if (Constants.contentTelemetry.size()>0) {
                if(Constants.contentTelemetry.get("courseid").getAsString() != ""){
                    telemetry.addProperty("courseid", Constants.contentTelemetry.get("courseid").getAsString()); //need to take a decision
                }
                else{
                    telemetry.add("courseid",null);
                }
                telemetry.addProperty("resid", Constants.contentTelemetry.get("resid").getAsString());//value has to be passed
                telemetry.addProperty("restype", Constants.contentTelemetry.get("restype").getAsString());
            ...
            if (mode.equalsIgnoreCase("offline")){
                String telemetryAppData = SharedPrefrence.getItem(context , Constants.telemetryTokenKey , "undefined");
//            Log.d("TelemetryPlayerActivity",SharedPrefrence.getItem(context , telemetryTokenKey , "undefined"));
                if(!telemetryAppData.equals("undefined") && !telemetryAppData.equals("") ){
                    ...
                    SharedPrefrence.setItem(context, Constants.telemetryTokenKey ,telemetryObejct.toString());
                }
                else{
                    ...
                    SharedPrefrence.setItem(context, Constants.telemetryTokenKey,telemetryHeader.toString());
                }
            }
            else{
...
//            Log.d("TelemetryPlayerActivity",SharedPrefrence.getItem(context , telemetryTokenKey , "undefined"));
    ...
    public void telemetryPage (Context context) {
        ...
        try{
            ...
            String telemetryAppData = SharedPrefrence.getItem(context , Constants.telemetryTokenKey , "");
...
//                playerDataArray.add(playerTelemetry);
                ...
                SharedPrefrence.setItem(context, Constants.telemetryTokenKey ,telemetryJSONObj.toString());
            }
            else{
                SharedPrefrence.setItem(context, Constants.telemetryTokenKey,telemetry.toString());
            }
...
//    }
    ...
    public static void pushDownloadTelemetryData(final Context context, String identifier, String downloadMode, String status) {
        ...
        String openRapContent = SharedPrefrence.getItem(context, Constants.openRapDownload,"");
        JsonParser parser = new JsonParser();
        ...
        SharedPrefrence.setItem(context, Constants.openRapDownload,telemetryObject.toString());
...
//        String telemetryData = telemetryDownloadJson.toString();
    ...
    private static void downloadTelemetry(final Context context, JsonObject telemetryDownloadJson) {
        ...
        ApiInterfaceJava apiInterface = RetrofitSingleton.getInstance(Constants.baseUrl);
        Log.d("TestCases3Service","TestCases3 " +SharedPrefrence.getItem(context, Constants.telemetryTokenKey,""));
