...
import ekstep.societal.sdk.LexDownloadManager;
import ekstep.societal.sdk.UtilityJava;
import ekstep.societal.sdk.models.ContentDownloadEntity;
...
public class DownloadContentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent != null) {
            mContentId = intent.getStringExtra(Constants.EXTRA_DOWNLOAD_RES_ID).replace("\"", "");
            Log.d("DownloadContentOP","ID: "+mContentId);
            mDownloadMode = intent.getStringExtra(Constants.EXTRA_DOWNLOAD_MODE);
...
//                    Log.d("DownloadContent",ex.getMessage());
    ...
    private void handleOpenRapDownload() {
        ...
        long downloadId = LexDownloadManager.getInstance(this)
                .enqueue(Constants.openRapUrl + "/public/lex-content/" + mContentId + ".lex", Constants.TMP_DIR_PATH, mContentId + ".lex", true);
        Log.d("openRapTesting","DownloadId: "+downloadId);
        DownloadStatusEntity downloadStatusEntity = new DownloadStatusEntity(
                mContentId,
                true,
                Constants.openRapUrl + "/public/" + mContentId + ".zip",
                "INITIATED",
                0,
                downloadId,
                0
...
//        showSizeAlert(contentJson.get("size").getAsString());
    ...
    public ContentDownloadEntity downloadContentNew(JsonObject contentJSON, ContentDownloadEntity entities) {
        ...
        contentJSON.addProperty("appIcon", Constants.STORAGE_BASE_PATH + Constants.DATA_DIR_PATH + identifier + ".image.png");
        ...
        if (contentJSON.get("contentType").getAsString().equals("Resource")) {
            ...
            String extension = Constants.MIME_TO_EXTENSION.get(mimeType);
            ...
            long downloadId = LexDownloadManager.getInstance(mContext)
                    .enqueue(getDownloadUrl(contentJSON), Constants.DATA_DIR_PATH, identifier + "." + extension, false);
...
//        }
    ...
    private String downloadableUrl(String url){
        ...
        String downloadUrl;
        if(uriPort != -1){
            downloadUrl = Constants.baseUrl +"/"+url.split(uriPort + "/")[1];
        }
        else{
            if(url.contains("http://private-")){
                downloadUrl = Constants.baseUrl+removeScript(url);
            }
            else if(url.startsWith("/")){
                downloadUrl = Constants.baseUrl+url;
            }
            else{
                downloadUrl = url;
            }
    ...
    public void downloadThumbnail(String thumbnailUrl, String identifier) {
        ...
        LexDownloadManager.getInstance(mContext).enqueue(downloadableUrl(thumbnailUrl), Constants.DATA_DIR_PATH, identifier + ".image.png", false);
    }

    private void createNoImageFile(Context context){
        File noImage = new File(mContext.getExternalFilesDir(""), Constants.DATA_DIR_PATH +".nomedia");
        if (!noImage.exists()) {
            try {
