package ekstep.societal.space.services;
...
//                Utils.changeToTheme(this, Utils.THEME_DEFAULT);
        ...
        ColorUtil.changeWindowColor(mContext);
    }
    ...
    public void TOKEN_OUTGOING (String identifier) throws Exception {
        ...
        HomeActivity.loginPage.post(new Runnable() {
            @Override
            public void run() {
                HomeActivity.loginPage.evaluateJavascript("getSessionId()",null);
...
//        });
        ...
        if (data.contains("logout=true")){
            ...
            HomeActivity.splashPage.post(new Runnable() {
                @Override
                public void run() {
                //    splashPage.loadUrl("https://login.microsoftonline.com/logout.srf");
                    HomeActivity.splashPage.loadUrl("https://login.microsoftonline.com/common/oauth2/logout?post_logout_redirect_uri=https%3A%2F%2F"+Constants.environmentType+".infosysapps.com");
                }
            });
...
//       });
        ...
        if(data.replaceAll("\"","").equalsIgnoreCase("yes")){
            HomeActivity.chatButton.post(new Runnable() {
                @Override
                public void run() {
                    HomeActivity.chatButton.setVisibility(View.VISIBLE);
                }
            });
        }
        else{
            HomeActivity.chatButton.post(new Runnable() {
                @Override
                public void run() {
                    HomeActivity.chatButton.setVisibility(View.GONE);
                }
            });
        }
